  ...
  return (
    <Layout>
      <section className="mx-auto flex min-h-[80vh] max-w-5xl items-center justify-center px-4 py-16 sm:px-6 lg:px-8">
        <div className="w-full max-w-xl rounded-3xl border border-slate-200 bg-white p-8 shadow-xl transition-colors dark:border-slate-800 dark:bg-slate-900">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-600 dark:text-emerald-400">Create account</p>
          <h1 className="mt-3 text-3xl font-bold text-slate-900 dark:text-slate-50">Register to start donating</h1>
          <form onSubmit={handleSubmit(onSubmit)} className="mt-8 grid gap-4 md:grid-cols-2">
            <div>
              ...
              />
              {errors.name && <p className="mt-1 text-sm text-rose-600 dark:text-rose-400">{errors.name.message}</p>}
            </div>
            <div>
            ...
            <div>
              ...
              <select
                {...registerField('role')}
                className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:focus:border-emerald-400"
              >
          ...
          <p className="mt-6 text-center text-sm text-slate-600 dark:text-slate-400">
            Already have an account? <Link to="/login" className="font-semibold text-emerald-600 hover:underline dark:text-emerald-400">Sign in</Link>
          </p>
        </div>
      </section>
    </Layout>
